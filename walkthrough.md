...
## Branding por panel (favicon + títulos)
- [ ] Abrir `/app` y validar favicon tenant + título `ORBIA - PANEL CENTRAL` (y títulos por sección: Configuración/Productos/Pedidos/Caja/Sucursales).
- [ ] Abrir `/delivery` y validar favicon delivery + título `ORBIA - DELIVERY`.
- [ ] Abrir `/owner` o `/super` y validar favicon admin + título `ORBIA - ADMINISTRACIÓN` (y `ORBIA - SEGURIDAD` en tab de seguridad).
- [ ] Confirmar que en ningún panel aparece el título del host (ej: `localhost:5000/...`).
